...
class _ActivityPageState extends State<ActivityPage>
  ...
  Future<void> _loadFeed() async {
    setState(() { _loadingFeed = true; _feedError = null; });
    try {
      ...
      if (acts.isEmpty) {
        if (mounted) setState(() { _items = []; _loadingFeed = false; });
        return;
      }
      ...
      for (final v in votes) {
        ...
        if (val == 1) appC[aid] = (appC[aid] ?? 0) + 1;
        else           rejC[aid] = (rejC[aid] ?? 0) + 1;
      ...
      final items = acts.map((act) {
        ...
        return _VotingItem(
          ...
          imageUrl:     preuveList.isNotEmpty ? (preuveList.first['url'] as String? ?? '') : '',
      ...
      if (mounted) setState(() { _items = items; _loadingFeed = false; });
    } catch (_) {
      if (mounted) setState(() { _feedError = 'Failed to load voting feed.'; _loadingFeed = false; });
    }
  }
  ...
  Future<void> _loadMyCompleted() async {
    ...
    final uid = _myId;
    if (uid == null) { setState(() => _loadingCompleted = false); return; }
    try {
      final data = await Supabase.instance.client
          .from('activite')
          .select('id_act, titre, localisation, datecreation, xpfinal, type_activite(nom), preuve(url)')
          .eq('status', 'approved')
          .eq('id_utilisateur', uid)
          .order('datecreation', ascending: false);

      final list = (data as List).cast<Map<String, dynamic>>().map(_parseSimple).toList();
      if (mounted) setState(() { _myCompleted = list; _loadingCompleted = false; _completedLoaded = true; });
    } catch (_) {
      if (mounted) setState(() { _loadingCompleted = false; _completedLoaded = true; });
    }
  }
  ...
  Future<void> _loadMyRejected() async {
    ...
    final uid = _myId;
    if (uid == null) { setState(() => _loadingRejected = false); return; }
    try {
      ...
      if (actIds.isEmpty) {
        if (mounted) setState(() { _myRejected = []; _loadingRejected = false; _rejectedLoaded = true; });
        return;
      }
      ...
      final list = (data as List).cast<Map<String, dynamic>>().map(_parseSimple).toList();
      if (mounted) setState(() { _myRejected = list; _loadingRejected = false; _rejectedLoaded = true; });
    } catch (_) {
      if (mounted) setState(() { _loadingRejected = false; _rejectedLoaded = true; });
    }
  }
              ...
              child: TabBarView(
                ...
                children: [
                  ...
                  _buildSimpleTab(
                    ...
                    rejected: false,
                    onRefresh: () async { _completedLoaded = false; await _loadMyCompleted(); },
                  ),
                  _buildSimpleTab(
                    ...
                    rejected: true,
                    onRefresh: () async { _rejectedLoaded = false; await _loadMyRejected(); },
                  ),
                ],
              ),
            ),
      ...
      child: _loadingFeed
          ...
          : _feedError != null
              ? Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  ElevatedButton(
                    onPressed: _loadFeed,
                    style: ElevatedButton.styleFrom(backgroundColor: _deepGreen, foregroundColor: Colors.white),
                  ...
                  ? ListView(children: [
                      SizedBox(
                        ...
                        child: Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
                          Icon(Icons.how_to_vote_outlined, color: Colors.grey.shade400, size: 64),
                          const SizedBox(height: 14),
                          Text('No activities awaiting votes.', style: TextStyle(color: Colors.grey.shade700, fontSize: 16, fontWeight: FontWeight.w600)),
                          const SizedBox(height: 6),
                          Text('Create an activity to get the community voting!', style: TextStyle(color: Colors.grey.shade500)),
                        ])),
                      ),
                    ])
                  : ListView.builder(
                  ...
                  SizedBox(
                    ...
                    child: Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
                      ...
                      Padding(
                        ...
                        child: Text(emptyMsg, textAlign: TextAlign.center, style: TextStyle(color: Colors.grey.shade700, fontSize: 15, fontWeight: FontWeight.w600)),
                      ),
                    ])),
                  ),
                ])
  ...
  Widget _buildSimpleCard(_SimpleActivity act, {required bool rejected}) {
    ...
    final badgeIcon  = rejected ? Icons.thumb_down_rounded : Icons.check_circle_rounded;

    return Container(
      decoration: BoxDecoration(
        color: Colors.white,
        borderRadius: BorderRadius.circular(20),
        boxShadow: const [BoxShadow(color: Color(0x141B5E20), blurRadius: 12, offset: Offset(0, 4))],
      ),
      child: Row(
        children: [
          ClipRRect(
                  ...
                  Row(children: [
                    Expanded(
                      child: Text(act.title,
                          ...
                          style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: Color(0xFF163217))),
                    ),
                    ...
                    Container(
                      ...
                      child: Row(mainAxisSize: MainAxisSize.min, children: [
                        ...
                        Text(badgeLabel, style: const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w700)),
                      ]),
                    ),
                  ]),
                  if (act.categoryName.isNotEmpty) ...[
                    const SizedBox(height: 4),
                    Text(act.categoryName, style: const TextStyle(fontSize: 11, color: Color(0xFF4A7D4E), fontWeight: FontWeight.w600)),
                  ],
                  const SizedBox(height: 6),
                  Wrap(spacing: 10, children: [
                    if (act.location.isNotEmpty) _metaChip(Icons.location_on_rounded, act.location),
                    if (act.date.isNotEmpty) _metaChip(Icons.calendar_today_rounded, act.date),
                    if (act.xpLabel.isNotEmpty) _metaChip(Icons.bolt_rounded, act.xpLabel),
                  ]),
                ],
              ),
            ),
  ...
  Widget _buildVoteCard(_VotingItem item) {
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        boxShadow: const [BoxShadow(color: Color(0x141B5E20), blurRadius: 16, offset: Offset(0, 6))],
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ClipRRect(
            ...
            child: Stack(children: [
              ...
              if (item.categoryName.isNotEmpty)
                Positioned(top: 10, left: 10, child: _badge(item.categoryName, const Color(0xFF1B5E20), Colors.white)),
              if (item.xpLabel.isNotEmpty)
                Positioned(top: 10, right: 10, child: _badge(item.xpLabel, const Color(0xFFFFB300), const Color(0xFF4A3800))),
                ...
                Positioned.fill(
                  child: Container(
                    ...
                    child: const Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
                      ...
                      SizedBox(height: 6),
                      Text('Voting Closed', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w700)),
                    ])),
                  ),
                ),
            ]),
          ...
          Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(item.title, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: Color(0xFF163217))),
                if (item.description.isNotEmpty) ...[
                  const SizedBox(height: 6),
                  Text(item.description, maxLines: 2, overflow: TextOverflow.ellipsis,
                      style: const TextStyle(fontSize: 13, color: Color(0xFF607060), height: 1.4)),
                ],
                const SizedBox(height: 10),
                Wrap(spacing: 12, runSpacing: 4, children: [
                  if (item.location.isNotEmpty)  _metaChip(Icons.location_on_rounded, item.location),
                  if (item.date.isNotEmpty)       _metaChip(Icons.calendar_today_rounded, item.date),
                  if (item.levelName.isNotEmpty)  _metaChip(Icons.military_tech_rounded, item.levelName),
                ]),
                const SizedBox(height: 16),
                _buildVoteProgress(item),
                const SizedBox(height: 14),
                if (isOwner)            _ownerBanner()
                else if (item.votingClosed)  _closedBanner(item)
                else if (item.hasVoted) _votedBanner(item)
                else                    _buildVoteButtons(item, isVoting),
              ],
            ),
          ),
        ],
  ...
  Widget _buildVoteProgress(_VotingItem item) {
    return Column(
      ...
      children: [
        Row(children: [
          Text('${item.totalVotes} / 2 votes',
              style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: Color(0xFF4A7D4E))),
          const Spacer(),
          _dot(const Color(0xFF2E7D32)), const SizedBox(width: 4),
          Text('${item.approveCount}', style: const TextStyle(fontSize: 12, color: Color(0xFF2E7D32), fontWeight: FontWeight.w700)),
          const SizedBox(width: 2),
          const Text('Approved', style: TextStyle(fontSize: 11, color: Color(0xFF4A7D4E))),
          const SizedBox(width: 10),
          _dot(const Color(0xFFB71C1C)), const SizedBox(width: 4),
          Text('${item.rejectCount}', style: const TextStyle(fontSize: 12, color: Color(0xFFB71C1C), fontWeight: FontWeight.w700)),
          const SizedBox(width: 2),
          const Text('Rejected', style: TextStyle(fontSize: 11, color: Color(0xFF8B4040))),
        ]),
        ...
        LayoutBuilder(builder: (_, constraints) {
          ...
          return ClipRRect(
            ...
            child: SizedBox(
              height: 10, width: full,
              child: Stack(children: [
                Container(width: full, color: const Color(0xFFE0E0E0)),
                if (appW > 0) Positioned(left: 0, top: 0, bottom: 0, child: Container(width: appW, color: const Color(0xFF2E7D32))),
                if (rejW > 0) Positioned(left: appW, top: 0, bottom: 0, child: Container(width: rejW, color: const Color(0xFFB71C1C))),
              ]),
            ),
          );
        }),
  ...
  Widget _buildVoteButtons(_VotingItem item, bool isVoting) {
    ...
    return Row(children: [
      Expanded(child: _voteBtn(label: 'Approve', icon: Icons.thumb_up_rounded,   color: _approveColor, onTap: () => _castVote(item.id,  1))),
      const SizedBox(width: 12),
      Expanded(child: _voteBtn(label: 'Reject',  icon: Icons.thumb_down_rounded, color: _rejectColor,  onTap: () => _castVote(item.id, -1))),
    ]);
  }

  Widget _voteBtn({required String label, required IconData icon, required Color color, required VoidCallback onTap}) {
  ...
  Widget _ownerBanner() => _infoBanner(
    ...
    bgColor: const Color(0xFFE8F5E9), borderColor: const Color(0xFF4CAF50),
    textColor: const Color(0xFF1B5E20), iconColor: const Color(0xFF2E7D32),
  );
  ...
  Widget _votedBanner(_VotingItem item) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: approved ? const Color(0xFF4CAF50) : const Color(0xFFE57373)),
      ),
      child: Row(children: [
        Icon(approved ? Icons.thumb_up_rounded : Icons.thumb_down_rounded,
            color: approved ? const Color(0xFF2E7D32) : const Color(0xFFB71C1C), size: 18),
        const SizedBox(width: 10),
        Expanded(child: Text(
          approved ? 'You voted: Approved' : 'You voted: Rejected',
          style: TextStyle(color: approved ? const Color(0xFF1B5E20) : const Color(0xFFB71C1C), fontWeight: FontWeight.w700, fontSize: 13),
        )),
        Text('Awaiting 2nd vote', style: TextStyle(color: Colors.grey.shade500, fontSize: 11)),
      ]),
    );
  }
  ...
  Widget _closedBanner(_VotingItem item) {
    ...
    return _infoBanner(
      ...
      text: w ? 'Voting closed – Activity Approved' : 'Voting closed – Activity Rejected',
  ...
  Widget _infoBanner({required IconData icon, required String text, required Color bgColor, required Color borderColor, required Color textColor, required Color iconColor}) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 12),
  ...
  Widget _imagePlaceholder() => Container(
    color: const Color(0xFFE8F5E9),
    child: const Center(child: Icon(Icons.eco_rounded, color: Color(0xFF4CAF50), size: 40)),
  );
  ...
  Widget _metaChip(IconData icon, String label) => Row(mainAxisSize: MainAxisSize.min, children: [
    ...
    const SizedBox(width: 4),
    Text(label, style: const TextStyle(fontSize: 12, color: Color(0xFF607060), fontWeight: FontWeight.w500)),
  ]);

  Widget _dot(Color color) => Container(width: 9, height: 9, decoration: BoxDecoration(color: color, shape: BoxShape.circle));
  ...
  Widget _buildHeader() {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: const [BoxShadow(color: Color(0x33244D2B), blurRadius: 18, offset: Offset(0, 8))],
      ),
      child: Row(children: [
        Container(
          width: 52, height: 52,
          decoration: BoxDecoration(shape: BoxShape.circle, color: const Color(0x22FFFFFF), border: Border.all(color: const Color(0x44FFFFFF), width: 2)),
          child: const Icon(Icons.how_to_vote_rounded, color: Colors.white, size: 28),
        ),
        const SizedBox(width: 14),
        const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Community Voting', style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w800)),
          SizedBox(height: 4),
          Text('Review & approve new eco activities', style: TextStyle(color: Color(0xFFB8DFB8), fontSize: 13)),
        ])),
      ]),
    );
  }
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          gradient: const LinearGradient(colors: [Color(0xFF4CAF50), Color(0xFF1B5E20)], begin: Alignment.topLeft, end: Alignment.bottomRight),
          border: Border.all(color: const Color(0xFFE8F7E7), width: 2),
          boxShadow: const [BoxShadow(color: Color(0x4427502E), blurRadius: 20, offset: Offset(0, 8))],
        ),
  ...
  Widget _navItem(IconData icon, int index) {
    ...
    return GestureDetector(
        ...
        if (index == 0) {
          Navigator.pushReplacement(context, PageRouteBuilder(
            ...
            transitionsBuilder: (_, anim, __, child) => FadeTransition(opacity: anim,
              child: SlideTransition(position: Tween<Offset>(begin: const Offset(-1, 0), end: Offset.zero)
      ...
      child: AnimatedContainer(
        ...
        width: 40, height: 40,
        decoration: BoxDecoration(borderRadius: BorderRadius.circular(12), color: active ? const Color(0xFFA5D6A7) : Colors.transparent),
        child: Icon(icon, color: active ? const Color(0xFF163D17) : Colors.white, size: 27),
      ),
    );
  }
}